...
country = 'IT'
quest = 'FinitePool_2'
fear_questions = c('CanadaQ_2', 'CanadaQ_1', 'Longitude_2', 'FinitePool_2', 'FinitePool_5', 'CanadaQ_3', 'Friends_8', 'Personal_8')
...
regression_data <- merge(compress_df(aggregate_data_func(controlling_for_questions)), fear_scores) %>%
  ...
  mutate(stringency_sq = (normalit(stringency)-0.5)^2)
...
mod <- fit(lm_spec,
            #fear_index ~ stringency_sq,
            fear_index ~ . -country -stringency,
...
regression_data %>%
  ggplot(aes(x=stringency, y=fear_index, color=country)) +
  geom_point()
