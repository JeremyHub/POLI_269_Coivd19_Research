...
put_new_lines <- function(string, num_chars) {
  ...
  first = substr(string, 1, num_chars)
  last = substr(string, num_chars+1, nchar(string))
...
for (i in 1:max(code_text$code)){
  ...
  if (i %in% code_text$code) {
    labels <- c(labels, paste(to_paste,put_new_lines(code_text[code_text$code==i,]$label,15)))
  } else {
    labels <- c(labels, i)
  }
}
